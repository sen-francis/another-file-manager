...
std::vector<std::string> backend::search(std::string folderPath, pair<std::string, std::string> createdDates, pair<std::string, std::string> modifiedDates){
  std::vector<std::string> files = new std::vector<std::string>();
  for (const auto & entry : std::filesystem::directory_iterator(folderPath)){
    std::string file = files.add(entry.path().u8string());
    struct stat t_stat;
    ...
    if(modifiedDates!=null){
      if(!dateRangeComparison(modifiedDate, modifiedDate)){
        // If date is not in between given date range, don't add file
        continue;
      }
    }
...
std::string backend::getMonthIndex(string month)
{
    map<std:string, std::string> months
    {
        { "Jan", "01" },
        { "Feb", "02" },
        { "Mar", "03" },
...
bool backend::dateRangeComparison(std::string date, pair<std::string, std::string> dateRange){
...
Napi::Array backend::searchWrapped(const Napi::CallbackInfo& info){
  ...
  std::string createdDatesFrom = info[1].ToString().Utf8Value();
  std::string createdDatesTo = info[2].ToString().Utf8Value();
  std::string modifiedDatesFrom = info[3].ToString().Utf8Value();
  std::string modifiedDatesTo = info[4].ToString().Utf8Value();

  // Call search function
  std::vector<std::string> results = backend::search(folderPath, std::make_pair(createdDatesFrom, createdDatesTo), std::make_pair(modifiedDatesFrom, modifiedDatesTo));

  // Iterate through vector to create Napi::Array based on results
  Napi::Array napiResults = Napi::Array::New(info.Env(), results.size());
  for (int i = 0; i < results.size(); i++){
    napiResults[i] = Napi::String::New(info.Env(), results[i]);
  }
  return napiResults;
}
...
{
  exports.Set("search", Napi::Function::New(env, backend::searrchWrapped));
  return exports;
}
